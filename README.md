...
| 批量下载输出方式 | `file` 每个文件单独发送 · `forward` 合并转发为聊天记录（主要 OneBot v11 支持，不支持时自动退回 `file`） | `file` |
| 批量下载上限 | 单次批量最多处理多少个文件（限制在 1-50） | `20` |
...
```
/cie getall 0580 s20            # 该考季全部（试卷+答案）
/cie getall 0580 s20 11-42      # 卷号 11~42
/cie getall 0580 s20 11-42 qp   # 卷号 11~42，仅试卷
/cie getall 0580 s20 11,12,21   # 指定卷号
```

- **卷号范围**：`11-42`（区间）、`11,12,21`（列表）、`11`（单个），省略则下载该考季全部
- **输出方式**：由配置项「批量下载输出方式」决定 —— `file` 逐个文件发送，`forward` 合并转发为一条聊天记录
- **数量上限**：受配置项「批量下载上限」限制（默认 20，最多 50），超出会只下载前 N 个

> 别名：`getall` / `batch` / `ga` / `all` / `批量` 均可。
